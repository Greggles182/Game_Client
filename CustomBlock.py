...
def create():
    ...
    for i in range(num):
        ...
        mapTMP = graphicdesign()
        for b, n in enumerate(mapTMP):
            if n == 0:
                mapTMP[b] = (255, 255, 255)
            elif n == 1:
                mapTMP[b] = (255, 0, 0)
            elif n == 2:
                mapTMP[b] = (0, 255, 0)
                ...
                mapTMP[b] = (154, 154, 154)
            elif n == 9:
                mapTMP[b] = (104.6, 104.6, 104.6)
        maps[i] = mapTMP
        # Convert the pixels into an array using numpy
        array = np.array(mapTMP, dtype=np.uint8)

        # Create a grayscale image
        grayscale_image = Image.fromarray(array, 'L')

        # Create a new image with the same size as the grayscale image
        colored_image = Image.new('RGB', grayscale_image.size)

        # Apply the color palette to the new image
        colored_image.paste(grayscale_image)

        # Save the colored image
        colored_image.save(f'./levels/img/TEMP{i}.png')
    ...
    if os.path.isfile(f"levels/{name}.pkl"):
        ...
        pass
    input("Now we need to draw the level.", False)
    # with open(f"levels/{name}.pkl", "wb"):
    #     pickle.dump({"data":[], "leveldata":[], "code": open(f"levels/code/{name}{i}.py", "rt").read(), "graphics":{"1":map}})
